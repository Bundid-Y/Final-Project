...
if ((string) ($currentUser['company_code'] ?? 'TNB') !== 'TNB' && !in_array((string) ($currentUser['role'] ?? 'user'), ['super_admin', 'admin', 'manager'], true)) {
...
$serviceHistory = get_tnb_service_history($pdo, (int) $currentUser['id']);
$quotations = get_tnb_user_quotations($pdo, (int) $currentUser['id']);
    ...
    <main class="profile-shell">
        ...
        <div class="profile-grid">
            <aside class="profile-card">
                <img src="<?php echo h((string) ($profile['avatar_url'] ?: '../img/company_logo/tnb_logo.webp')); ?>" alt="User Avatar" class="profile-avatar">
                <h1 class="profile-name"><?php echo h(trim((string) ($profile['first_name'] ?? '') . ' ' . (string) ($profile['last_name'] ?? ''))); ?></h1>
                <p class="profile-meta"><?php echo h((string) ($profile['role'] ?? 'user')); ?> · <?php echo h((string) ($profile['company_name'] ?? 'TNB')); ?></p>
            ...
            <section>
                    ...
                    <form action="../../admin/api/profile/update.php" method="POST">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="field">
                                ...
                                <input type="text" id="company_name" value="<?php echo h((string) ($profile['company_name'] ?? 'TNB')); ?>" readonly>
                            </div>
                            <div class="field">
                        ...
                        <div class="actions">
                            ...
                            <a href="../../admin/api/auth/logout.php?company=tnb" class="btn secondary">Logout</a>
                        </div>
                    </form>
                </div>
                ...
                <div class="panel-card">
                    <h2 style="margin-top:0;">Service History</h2>
                    <div class="table-wrap">
                        <table>
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                <?php if ($serviceHistory === []): ?>
                                    <tr><td colspan="4">No service history found yet.</td></tr>
                                <?php else: ?>
                                    <?php foreach ($serviceHistory as $service): ?>
                                        <tr>
                                            <td><?php echo h((string) $service['created_at']); ?></td>
                                            <td><?php echo h((string) $service['service_type']); ?></td>
                                            <td><?php echo h((string) ($service['route'] ?? '-')); ?></td>
                                            <td><?php echo h((string) $service['status']); ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </tbody>
                ...
                <div class="panel-card">
                    ...
                    <div class="table-wrap">
                        <table>
                            ...
                            <tbody>
                                <?php if ($quotations === []): ?>
                                    <tr><td colspan="6">No quotations found yet.</td></tr>
                                <?php else: ?>
                                    <?php foreach ($quotations as $quotation): ?>
                                        <tr>
                                            <td><?php echo h((string) $quotation['request_number']); ?></td>
                                            <td><?php echo h((string) $quotation['created_at']); ?></td>
                                            <td><?php echo h((string) $quotation['service_type']); ?></td>
                                            <td><?php echo h((string) ($quotation['route'] ?? '-')); ?></td>
