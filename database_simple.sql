-- =============================================
-- Database: koch_tnb_system (Simple Version - No Stored Procedures)
...
SELECT 
    ...
    SUM(CASE WHEN quoted_price IS NOT NULL THEN quoted_price ELSE 0 END) as total_revenue
FROM tnb_quotations
GROUP BY DATE(created_at), status, service_type, truck_type;
...
/*
1. สร้าง database: สร้างอัตโนมัติในไฟล์นี้
2. นำเข้า SQL นี้: mysql -u root -p < database_simple.sql
...
Default Admin Login:
- Username: admin
- Email: admin@koch-tnb.com
- Password: password (ควรเปลี่ยนหลังติดตั้ง)
*/
